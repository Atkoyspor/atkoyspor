...
class SupabaseService {
            ...
            try {
                ...
                if (!error && users && users.length > 0) {
                    ...
                    console.log('Found user:', { 
                        ...
                        stored_password: user.password?.substring(0, 10) + '...' // Show first 10 chars for debug
                    });
            ...
            if (error) {
                ...
                if (error.code === 'PGRST116') {
                    ...
                    const defaultProfile = {
                        ...
                        role: user?.email === 'admin@atkoy.com' ? 'admin' : 'user',
                        phone: null
                    };
    ...
    async getEquipmentTypes() {
        try {
            if (!this.supabase) this.initialize();
            const { data: eqTypes, error } = await this.supabase
                .from('equipment_types')
                .select('*')
                .order('created_at', { ascending: false });
            if (error) throw error;
            ...
            for (const eq of (eqTypes || [])) {
                ...
                // assigned total
                const { data: assignedRows, error: assErr } = await this.supabase
                    .from('equipment_assignments')
