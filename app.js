...
class SportsManagementApp {
    ...
    async handleStudentFormSubmit(e, studentId = null) {
        ...
        const studentData = {
            ...
            // Also set branch for compatibility
        };

        // Validation
        if (!studentData.name || !studentData.surname || !studentData.tcno || !studentData.sport) {
        ...
        try {
            ...
            if (!input) {
                ...
                input.addEventListener('input', () => {
                    clearTimeout(this._studentSearchDebounce);
                    this._studentSearchDebounce = setTimeout(() => {
                        this._studentSearchQuery = (input.value || '').toLowerCase();
                        this.applyStudentFilters();
                    }, 150);
                });
            }
                ...
                if (!document.getElementById('equipmentGlobalSearchInput')) {
                    try {
                        ...
                        if (actions && tabs) {
                            ...
                            bar.innerHTML = '<i class="fas fa-search" style="position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #9CA3AF;"></i>' +
                                '<input id="equipmentGlobalSearchInput" type="text" placeholder="Ara: öğrenci, ekipman, beden..." style="width: 100%; padding: 10px 12px 10px 36px; border: 1px solid #e5e7eb; border-radius: 10px; background: white; font-size: 14px; color: #111827;">';
